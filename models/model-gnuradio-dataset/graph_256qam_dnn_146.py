...
json_file = open('model_256qam_146d.json', 'r')
loaded_model_json = json_file.read()
json_file.close()
loaded_model = model_from_json(loaded_model_json)
# load weights into new model
loaded_model.load_weights("model_256qam_146d.h5")
...
pyplot.plot(X, y_scaler.inverse_transform(predicted), linestyle=(0,(3,3)), linewidth=3, color="red")
pyplot.grid()
pyplot.xlabel('Função MSE Modificada [dB]', fontsize=14)
pyplot.ylabel('Saída DNN [dB]', fontsize=14)
pyplot.xticks(np.arange(-25,45,5), fontsize=12)
pyplot.yticks(fontsize=12)
pyplot.legend(('Dado', 'Estimado'), fontsize=14, loc='upper right')
pyplot.savefig('snr_256qam_dnn146.pdf')
pyplot.show()